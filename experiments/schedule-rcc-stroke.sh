...
mkdir -p $SCRATCH/Logs/rcc-stroke/$TIMESTAMP/
...
while read subject; do
  # Process the subject
  # sbatch $SCRATCH/Jobs/launch-rcc-stroke.sh $subject $FOLDER_SPEC $DATA_DIR $RESULTS_DIR
  ...
  sbatch --job-name="$FOLDER_SPEC" --account="plgsano4-cpu" --partition="plgrid" --output="$LOG_OUTPUT_DIR" --error="$LOG_ERROR_DIR" --time="72:00:00" --nodes="1" --ntasks-per-node="1" --cpus-per-task="1" --mem-per-cpu="1GB" $SCRATCH/Files/GitRepos/$GITREPO/experiments/rcc-stroke.sh $subject $DATA_DIR $RESULTS_DIR
done < "$subjects_list"
